...
class WeightedBothLocalizationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    predicted_boxes = box_list.BoxList(tf.reshape(prediction_tensor, [-1, 4]))
    target_boxes = box_list.BoxList(tf.reshape(target_tensor, [-1, 4]))
    matched_biou = box_list_ops.matched_biou(predicted_boxes, target_boxes)
    per_anchor_biou_loss = 1.0 - matched_biou
    return tf.reshape(weights, [-1]) * per_anchor_biou_loss
