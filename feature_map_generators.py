...
def rfpn_top_down_feature_maps(image_features,
                              depth,
                              use_depthwise=False,
                              use_explicit_padding=False,
                              use_bounded_activations=False,
  ...
  with tf.name_scope(scope, 'top_down'):
    num_levels = len(image_features)
# change 1
    second_feature_maps_list = []
    output_feature_maps_list = []
    output_feature_map_keys = []

    for level1 in range(num_levels):
      output_feature_map_keys.append('top_down_%s' % image_features[level1][0])
    ...
    with slim.arg_scope(
      ...
      if use_bounded_activations:
        top_down = tf.clip_by_value(top_down, -ACTIVATION_BOUND,
                                    ACTIVATION_BOUND)
      second_feature_maps_list.append(top_down)
      ...
      for level in reversed(range(num_levels - 1)):
        if use_native_resize_op:
          ...
          top_down = ops.nearest_neighbor_upsampling(top_down, scale=2)
        residual1 = slim.conv2d(
            image_features[level][1], depth, [1, 1],
            activation_fn=None, normalizer_fn=None,
            scope='projection_%d' % (level + 1))
        if use_bounded_activations:
          residual1 = tf.clip_by_value(residual1, -ACTIVATION_BOUND,
                                      ACTIVATION_BOUND)
        if use_explicit_padding:
          # slice top_down to the same shape as residual
          residual_shape1 = tf.shape(residual1)
          top_down = top_down[:, :residual_shape1[1], :residual_shape1[2], :]
        top_down += residual1
        if use_bounded_activations:
          top_down = tf.clip_by_value(top_down, -ACTIVATION_BOUND,
                                      ACTIVATION_BOUND)
    ...
    # output layer
    with slim.arg_scope(
        [slim.max_pool2d], padding=padding):
      bottom_up = second_feature_maps_list[-1]
      if use_bounded_activations:
        bottom_up = tf.clip_by_value(bottom_up, -ACTIVATION_BOUND,
                                    ACTIVATION_BOUND)
      output_feature_maps_list.append(bottom_up)

      for level in reversed(range(num_levels - 1)):
        if use_explicit_padding:
          bottom_up = ops.fixed_padding(bottom_up, 2)
        bottom_up = slim.max_pool2d(bottom_up, kernel_size=[2,2], stride=2)
        residual = second_feature_maps_list[level]
        if use_bounded_activations:
          residual = tf.clip_by_value(residual, -ACTIVATION_BOUND,
                                      ACTIVATION_BOUND)
        if use_explicit_padding:
          # slice top_down to the same shape as residual
          residual_shape = tf.shape(residual)
          #if(residual_shape[1] <= bottom_up[1]):
          bottom_up = bottom_up[:, :residual_shape[1], :residual_shape[2], :]
          #else:
    ...
    with slim.arg_scope(
        [slim.conv2d, slim.separable_conv2d], padding=padding, stride=1):
      top_down_fin = output_feature_maps_list[-1]
      for level in reversed(range(num_levels - 1)):
        if use_native_resize_op:
          with tf.name_scope('nearest_neighbor_upsampling'):
            top_down_shape_fin = shape_utils.combined_static_and_dynamic_shape(
                top_down_fin)
            top_down_fin = tf.image.resize_nearest_neighbor(
                top_down_fin, [top_down_shape_fin[1] * 2, top_down_shape_fin[2] * 2])
        else:
          top_down_fin = ops.nearest_neighbor_upsampling(top_down_fin, scale=2)
        residual2 = second_feature_maps_list[num_levels - level - 1]
        if use_explicit_padding:
          # slice top_down to the same shape as residual
          residual_shape2 = tf.shape(residual2)
          top_down_fin = top_down_fin[:, :residual_shape2[1], :residual_shape2[2], :]
        top_down_fin += residual2
        top_down_fin += output_feature_maps_list[level]
        if use_depthwise:
          conv_op = functools.partial(slim.separable_conv2d, depth_multiplier=1)
        else:
          conv_op = slim.conv2d
        if use_explicit_padding:
          top_down_fin = ops.fixed_padding(top_down_fin, kernel_size)
        output_feature_maps_list[level] = conv_op(
            top_down_fin,
            depth, [kernel_size, kernel_size],
            scope='smoothing_%d' % (level + 1))
