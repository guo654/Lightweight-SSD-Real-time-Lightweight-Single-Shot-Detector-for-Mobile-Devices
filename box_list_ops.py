...
def matched_diagonal(boxlist1, boxlist2, scope=None):
  """Compute the smallest axis-aligned bounding box that fully contains
     a pair of corresponding boxes from two boxlists.
  Args:
    boxlist1: BoxList holding N boxes
  ...
  Returns:
    a tensor with shape [N] representing pairwise intersections
  """
  with tf.name_scope(scope, 'MatchedDiagonal'):
    y_min1, x_min1, y_max1, x_max1 = tf.split(
        value=boxlist1.get(), num_or_size_splits=4, axis=1)
    y_min2, x_min2, y_max2, x_max2 = tf.split(
        value=boxlist2.get(), num_or_size_splits=4, axis=1)
    ...
    min_xmin = tf.minimum(x_min1, x_min2)
    containing_widths = tf.maximum(0.0, max_xmax - min_xmin)
    c = containing_heights * containing_heights + containing_widths * containing_widths
    l21 = tf.abs(y_max2 - y_max1)
    l22 = tf.abs(x_min2 - x_min1 )
    l23 = tf.abs(y_min2 - y_min1)
    l24 = tf.abs(x_max2 - x_max1)
    l2 = 2 * l21 * l21 + 2 * l22 * l22 + 2 * l23 * l23 + 2 * l24 * l24
    distance = tf.div_no_nan(l2, c )
    return tf.reshape(distance, [-1])
...
def matched_giou(boxlist1, boxlist2, scope=None):
  ...
  with tf.name_scope(scope, 'MatchedGIoU'):
    ...
    iou = tf.where(
          tf.equal(intersections, 0.0),
          tf.zeros_like(intersections), tf.truediv(intersections, unions))
    containings = matched_containing(boxlist1, boxlist2)
...
def matched_biou(boxlist1, boxlist2, scope=None):
  """Compute generalized intersection-over-union between corresponding boxes in boxlists.
  ...
  Returns:
    a tensor with shape [N] representing pairwise iou scores.
  """
  with tf.name_scope(scope, 'MatchedBIoU'):
    intersections = matched_intersection(boxlist1, boxlist2)
    areas1 = area(boxlist1)
    areas2 = area(boxlist2)
    unions = areas1 + areas2 - intersections
    iou = tf.where(
      tf.equal(intersections, 0.0),
      tf.zeros_like(intersections), tf.truediv(intersections, unions))
    l2part = matched_diagonal(boxlist1,boxlist2)
    biou = tf.subtract(iou, l2part)
    return biou
