...
######change1
@slim.add_arg_scope
def mixexpanded_conv(input_tensor,
                  num_outputs,
                  expansion_size=expand_input_by_factor(6),
                  stride=1,
                  rate=1,
  ...
  # pylint: disable=g-backslash-continuation
#change2
  with tf.variable_scope(scope, default_name='mixexpanded_conv') as s, \
       tf.name_scope(s.original_name_scope), \
      slim.arg_scope((slim.conv2d,), **conv_defaults), \
       slim.arg_scope((slim.separable_conv2d,), **dw_defaults):
    prev_depth = input_tensor.get_shape().as_list()[3]
...
#change3
    if depthwise_location == 'expansion':
      output_channels = []
      split_inputs = tf.split(net,2,axis = 3)
      if use_explicit_padding:
        split_inputs[0] = _fixed_padding(split_inputs[0],kernel_size, rate)
      net = depthwise_func(split_inputs[0])
      output_channels.append(net)
      if use_explicit_padding:
        split_inputs[1] = _fixed_padding(split_inputs[1],[5,5],rate)
      net = slim.separable_conv2d(split_inputs[1],num_outputs=None,kernel_size=(5,5),depth_multiplier=depthwise_channel_multiplier,stride=stride,rate=rate,normalizer_fn=normalizer_fn,padding=padding)
      output_channels.append(net)
      net = tf.concat(output_channels,axis = 3)
...
def spatial_attention(input_tensor,
  ...
  with tf.variable_scope('spatial_attention'):
    ...
    #if use_explicit_padding:
      #concat = _fixed_padding(concat, kernel_size, 1)
    concat = tf.layers.conv2d(concat, filters=1, kernel_size=[kernel_size,kernel_size], strides=[1,1],padding='same')
    #print(concat.get_shape())
    concat = gating_fn(concat)
    result = input_tensor * concat
