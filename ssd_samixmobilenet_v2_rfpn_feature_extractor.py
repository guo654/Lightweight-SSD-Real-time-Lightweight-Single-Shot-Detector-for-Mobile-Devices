...
from nets.mobilenet import samixmobilenet_v2
...
def _create_modified_mobilenet_config():
  conv_defs = copy.deepcopy(samixmobilenet_v2.V2_DEF)
  conv_defs['spec'][-1] = mobilenet.op(
      slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=256)
  return conv_defs


class SSDSaMixMobileNetV2RfpnFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
  ...
  def __init__(self,
    ...
    Args:
      ...
      override_base_feature_extractor_hyperparams: Whether to override
        ...
        `conv_hyperparams_fn`.
    """
    super(SSDSaMixMobileNetV2RfpnFeatureExtractor, self).__init__(
  ...
  def extract_features(self, preprocessed_inputs):
    ...
    with tf.variable_scope('SaMixmobilenetV2', reuse=self._reuse_weights) as scope:
      with slim.arg_scope(
          samixmobilenet_v2.training_scope(is_training=None, bn_decay=0.9997)), \
          slim.arg_scope(
              [mobilenet.depth_multiplier], min_depth=self._min_depth):
        with (slim.arg_scope(self._conv_hyperparams_fn())
              if self._override_base_feature_extractor_hyperparams else
              context_manager.IdentityContextManager()):
          _, image_features = samixmobilenet_v2.mobilenet_base(
      ...
      with slim.arg_scope(self._conv_hyperparams_fn()):
        with tf.variable_scope('rfpn', reuse=self._reuse_weights):
          feature_blocks = [
          ...
          fpn_features = feature_map_generators.rfpn_top_down_feature_maps(
