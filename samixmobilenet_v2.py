...
from nets.mobilenet import samixconv_blocks as ops
...
spatial_attention = ops.spatial_attention
_sa4 = lambda expansion_tensor, input_tensor: spatial_attention(expansion_tensor)

def mbv2_op(n, s=1, act=tf.nn.relu, sa=None, **kwargs):
  ...
  Args:
    ...
    **kwargs: passed to expanded_conv
    记住定义的时候如果已经预先定义了一些参数在defaults里面，就不能在函数中又定义一遍，会对输出的h和w有影响
  Returns:
    An object (lib._Op) for inserting in conv_def, representing this operation.
  """
  return op(
      ops.mixexpanded_conv,
      stride=s,
      num_outputs=n,
      inner_activation_fn=act,
      expansion_transform=sa,
      **kwargs)

mbv2_op_sa = functools.partial(mbv2_op, sa=_sa4)
...
V2_DEF = dict(
    defaults={
        ...
        (ops.expanded_conv,ops.mixexpanded_conv): {
    ...
    spec=[
        ...
        #op(ops.mixexpanded_conv, stride=1, num_outputs=32),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=32),
        #op(ops.mixexpanded_conv, stride=2, num_outputs=64),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=64),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=64),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=64),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=96),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=96),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=96),
        #op(ops.mixexpanded_conv, stride=2, num_outputs=160),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=160),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=160),
        #op(ops.mixexpanded_conv, stride=1, num_outputs=320),
        mbv2_op_sa(s=2, n=32),
        mbv2_op_sa(s=1, n=32),
        mbv2_op_sa(s=1, n=32),
        op(ops.mixexpanded_conv, stride=2, num_outputs=64),
